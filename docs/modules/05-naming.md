...
**`generate_zunhao()`** —— 登基初上尊号：天命池一段 + 功德义类一段（如「应天神武皇帝」），
以 `used_zunhao` 局内查重（编年事件曾按尊号归档，撞号会致事件张冠李戴）；二段组合
耗尽时加第三段兜底。同时初始化 `emperor_zunhao_frags`（碎片列表）与
`zunhao_last_add_year`（上次上号之年，绝对纪年）。

**`try_offer_zunhao()`** —— 在位加号礼，每年由 `events.event_happen` 末尾调用：

- **触发**：距上次上号 ≥`ZUNHAO_ADD_MIN_YEARS`（8 年），且当年为治世（国运 ≥60）
  或大功之年（本年国运变化 ≥+5）；基础概率 5%，大功 +15%，国运 ≥85 再 +5%。
- **辞让**：勤政 ≥60 时 35% 概率（否则 15%）固辞不受，国祚 +1，记「帝谦抑固辞」。
- **受号**：按情势择义类（大捷→武功；治世→文治/德行；圣主→道蕴），抽一段未用
  碎片累加，全称查重登记；同步 `emperor_zunhao` 与今上 `Person.zunhao`；礼成
  靡费国祚 −1，记「御殿受册，大赦天下」。
- **上限**：`ZUNHAO_MAX_SEGMENTS`（6 段）后不再加号。

调参：碎片池在 `resources.zunhao_theme_pools`；间隔/上限为 `NamingMixin` 类常量；
触发与辞让概率在 `try_offer_zunhao` 内。
